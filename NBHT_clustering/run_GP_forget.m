function out = run_GP_forget(x,y,params,predict_on)
...
%gp_Detect_Fault = 10;
gpc = onlineGP.onlineGP(bandwidth,noise,max_points,tol);
gpc.set('reg_type','regularize');

test_x = zeros(size(x));
test_x = [test_x; zeros(1,size(test_x,2))];
test_y = zeros(size(y));
mu = zeros(1,length(y));
sigma = mu;
i=1;
act_model = 1;
forget = 120; %corresponds to 1/2 at 100 and b =1
forget = 30;
while 1
