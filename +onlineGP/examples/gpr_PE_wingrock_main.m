...
% add path to onlineGP folder, data and utils
addpath('../../')
addpath('./data')
addpath('./utils')
...
wn=0.03^2;% measurement noise covariance
...
gpr = onlineGP.onlineGP_PE(bandwidth,noise,max_points,tol,1,0.02);
%gpr = onlineGP.onlineGP(bandwidth,noise,max_points,tol);
...
paramHist = [ones(length(t),1)*bandwidth,ones(length(t),1)*noise] ;
...
for t=t0:dt:tf
    ...
    %EDIT: should be sqrt(wn) not wn
    x=x+randn(2,1)*sqrt(wn); % measurement noise is added
    ...
    if gp_on
        ...
        if length(gpr.get('obs')) >= 10 && updateParams == 1
            gpr.update_param(x,deltaErrNoisy);
            paramHist(index,1) = gpr.get('sigma');
            paramHist(index,2) = gpr.get('snr');
        end
    end
...
NMSE_ONLINE = mean(abs(DELTAERR_REC-VAD_REC))./std(DELTAERR_REC) %online error
%error for post learning
%NMSE_POST = mean(abs(DELTAERR_REC-VAD_POST))./std(DELTAERR_REC) %post error
...
if gp_on
    ...
    figure(9)
    plot3(XRM_REC,XDOTRM_REC,VAD_REC,'x g')
    hold on
    plot3(XRM_REC,XDOTRM_REC,DELTAERR_REC,'x')
    
    if updateParams ==1
        figure(10)
        plot(T_REC,paramHist(:,1))
        hold on
        plot(T_REC,paramHist(:,2)*100,'g')
        legend('Bndwidth','noise')
