...
%experiment parameters
N_eps = 300;%100; %200 Number of episodes
N_exec = 1; %20;
                ...
                %use the correct action
                  ...
                  %from GP2, this is really a constant
                   prob_not_in_set = -1/2* ((mu_IS2'-mu_IS2')*S_IS2^-1*(mu_IS2-mu_IS2) + log( det(S_IS2)))- detect_size/2*log(2*pi) ;
                        ...
                        %fprintf('KL was %f \n', KL_div);
                        %KL_div_hist(jj,k) = KL_div;
                        mu_diff = mu_IS2-mu_IS1;
                        if((KL_div > epsilon1))% && (sum(mu_diff <-0.1) > 1))
                            ...
                            for x=1:params.N_grid
                               for y=1:params.N_grid
                                    plotQ(x,y)= Q_value(theta2,[x;y],1,params,gpr2);
                                    plotPol(x,y) = 1;
                                    for a=2:params.N_act
                                        if(Q_value(theta2,[x;y],a,params,gpr2) > plotQ(x,y))
                                            plotQ(x,y)= Q_value(theta2,[x;y],a,params,gpr2);
                                            plotPol(x,y) = a;
                                        end
                                    end
                                   % plotPolX(x,y) = plot
