%This one does point wise checks
%things to branch on
%1) environment
%2) initial state
%3) dimensions, 
...
%GP params'
 ...
 elseif(strcmp(params.env,'grid') == 1)
    ...
    if(updateStyle == 2) %cautious
        epsilon1 = 100; %threshold basis points needs to come down by before we do a check
    end
...
%experiment parameters
N_eps = 10; %200 Number of episodes
N_exec = 1; %20;
...
for j = 1:N_eps
    ...
    for k = 1: N_eps_length
        ...
        if(1)    %TJW: this was there.  Rob was scared.  act_val2 + act_val_var2 < act_val1 - act_val_var1)
            ...
            elseif(updateStyle ==3 || updateStyle == 2) %KL divergence check on basis vectors      
                ...
                %use the correct action
                    ...
                    %of actions
                    B(1:params.N_state_dim,1:csa2) = BV2(:,1:csa2);  %(:,((jj-1)*params.N_budget+1):((jj-1)*params.N_budget+csa1));  
                   ...
                   newMu{jj} = mu_IS1;
                   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                   %%%%%% New check for each BV
                   for ind = 1:size(mu_IS1,2)
                       %changed by epsilon1?
                       if mu_diff(ind) < -epsilon1
                           %confident?
                           if S_IS2(ind,ind) < 0.5/2
                               newMu{jj}(ind,1) = mu_IS1(ind)-epsilon1;
                               swap = 1;
                           end
                       end
                   end
                ...
                if swap
                    ...
                    fprintf('significant change!  Making the swap!!!!!!!!!\n');
                    gpr2.reinitCovar(params);
                    [params,gpr1,theta1] = QL_copy(params,gpr2,theta2,newMu);
                    %gpr2.reinitCovar(params);
                    plotme = 1;
                    %adaptive epsilon
        ...
        if plotme 
            ...
            for jj = 1:5
                BB = ones(params.N_state_dim +1,25) * jj;     
                [XX YY] = meshgrid(1:5);
                BV1B = [XX(:)'; YY(:)'];
                BB(1:params.N_state_dim,:) = BV1B;
                mu_B =  gpr1.predict(BB,params);
                if mu_B == 0
                    mu_B = zeros(1,25);
                end
