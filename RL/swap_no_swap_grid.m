...
N_eps = 75; %200 Number of episodes
N_exec = 10; %20;
...
for j = 1:N_eps
    ...
    for k = 1: N_eps_length
        ...
        [rew,breaker] = gridworld_rewNeg(s_new,params);
        rew = rew - RMAX;
    ...
    disp(['steps:',num2str(train_Steps(1,j))]);
...
rews(ex,:) = train_Steps(1,:);%train_Reward(1,:);
...
filename = ['cautious_swap'];  %'result_m',num2str(nm),'bw',num2str(nbw),'am',num2str(na),'nn',num2str(nn),'bg',num2str(nbg),'to',num2str(nt),'r',num2str(i)];
if(updateStyle == -1)
    filename = ['no_swap'];
end
if(updateStyle == 1)
